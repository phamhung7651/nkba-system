              ...
              <div key={rep.id} className="bg-white border border-slate-200 rounded-[2.5rem] flex flex-col overflow-hidden hover:shadow-2xl transition-all group relative">
                ...
                {!hasAccess && (
                  <div className="absolute inset-0 bg-slate-900/60 backdrop-blur-md z-20 flex flex-col items-center justify-center p-8 text-center text-white">
                    ...
                    <button className="px-5 py-2 bg-amber-500 text-[#002D62] rounded-lg font-black text-[10px] uppercase">Nâng cấp ngay</button>
                  </div>
                )}

                <div className={`h-48 bg-gradient-to-br from-slate-50 to-slate-200 flex items-center justify-center relative ${!hasAccess ? 'opacity-30' : ''}`}>
                  <i className="ph-fill ph-file-pdf text-7xl text-slate-300 group-hover:scale-110 transition-transform duration-500"></i>
                  <div className="absolute top-5 left-5">
                    <span className="text-[9px] font-black px-3 py-1.5 bg-white rounded-lg border border-slate-200 uppercase tracking-widest shadow-sm">
                      {rep.access_tier}
                    </span>
                  </div>
                </div>
                ...
                <div className="p-7 flex-1 flex flex-col">
                  ...
                  <div className="mt-auto pt-8">
                    <button disabled={!hasAccess} className="w-full h-12 bg-[#002D62] text-white rounded-2xl text-xs font-black hover:bg-black transition-all flex items-center justify-center gap-2 disabled:bg-slate-100 disabled:text-slate-300">
                      <i className="ph-bold ph-download-simple text-lg"></i> TẢI BÁO CÁO
                    </button>
                  </div>
                </div>
              </div>
